...
| camera_key | String(32) UNIQUE | 認証キー（自動生成） |
| is_active | Boolean | 有効/無効 |
| pending_command | String(50) NULL | reboot等の一発コマンド |
...
| id | Integer PK | |
| camera_id | FK → cameras | |
| command | String(50) | "start" / "stop" / "reboot" |
...
| `cameras/{camera_key}/command` | クラウド → カメラ | No | reboot等の一発コマンド |
...
| JWT | POST | /api/cameras/{id}/reboot | Rebootコマンド（→MQTT配信） |
...
- カメラ詳細:
  ...
  - **Stream Control**: Desired State、Actual State、Start/Stop/Rebootボタン
