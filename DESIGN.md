...
```
現在:  Jetson → MQTT → クラウド → DB保存 → APIポーリング → ブラウザ
改善案1: Jetson → MQTT → Redis(インメモリ) → APIポーリング → ブラウザ
改善案2: Jetson → MQTT → ブラウザ(MQTT直接接続、DB経由しない)
```
- 案1（Redis）: サーバー側の変更のみ、フロント変更なし
- 案2（MQTT直接）: DB/API不要でサーバー負荷ゼロ、ただしフロント改修必要
- ステータスは履歴不要の揮発データなのでDBに入れる必要がない
