...
├── app/
│   ├── main.py                   # FastAPIアプリ + MQTT起動 + offline_checker
│   ├── config.py
│   ├── database.py
...
│   │   ├── user.py
│   │   ├── company.py
│   │   └── camera.py             # Camera, CameraSettings, CameraStatus, CommandLog
│   ├── schemas/
...
3. **ステータス報告**: camera-api → jetson_client.py → MQTT Publish → クラウドDB更新
4. **再接続**: Retained Messageにより最新設定を即座に取得
5. **オフライン検知**: last_seenが10秒以上前 → is_online=False（5秒間隔チェック）
...
### ステータス管理の改善（台数増加時）
現在はカメラステータス（CPU/GPU/ビットレート等）を2秒ごとにDBにUPDATE。
数台なら問題ないが、台数が増えるとDB負荷が大きくなる。
（PostgreSQL: 毎秒~1,500 UPDATEが限界付近、Redis: 毎秒100,000+余裕）

```
現在: Jetson → MQTT → DB(PostgreSQL) ← API ← ブラウザ
将来: Jetson → MQTT → Redis          ← API ← ブラウザ/スマホ
```
- DBをRedisに置き換えるだけ（APIもフロントも変更不要）
- スマホアプリからもAPIで全ステータス取得可能
- ステータスは履歴不要の揮発データなのでインメモリで十分

#### DBとRedisの役割分担
| 保存先 | データ | 例 |
|---|---|---|
| DB(PostgreSQL) | 永続データ | ユーザー、企業、カメラ、設定、コマンド履歴 |
| Redis | 揮発データ（リアルタイムステータス） | CPU/GPU/メモリ/温度/ビットレート/FPS等 |

#### DBに残すステータス項目
| 項目 | 理由 |
|---|---|
| is_online | カメラ一覧表示、オフライン判定 |
| last_seen | オフライン判定（10秒ルール） |
| stream_running | 配信中かどうか（Actual State、APIで取得可能にする） |
